...
from showtime.database import get_memory_db
...
def test_last_seen():
    ...
    with get_memory_db("") as database:
        show_id = database.add(show1)
        database.add_episode(show_id, episode1)
        database.add_episode(show_id, episode2)
        database.add_episode(show_id, episode3)
